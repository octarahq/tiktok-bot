    ...
    while (!accepted) {
        ...
        const choice = await p.select({
            ...
            options: [
                ...
                { value: 'retry', label: '🔄 Régénérer un autre sujet' },
            ],
        });
        ...
        if (choice === 'guide') {
            ...
            subject = await GenerateSubject(oldSubjects, {
                system: system + ` Here is some additional guidance from the user: ${safeHint}`,
            });
        } else if (choice === 'yes') {
            accepted = true;
        }
...
async function GenerateSubject(oldSubjects: string[], options?: { system?: string }): Promise<string> {
